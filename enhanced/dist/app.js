(function () {
    const STORAGE_KEY = 'eris_preview_tetris_v2';
    const SETTINGS_DEFAULTS = { dasMs: 110, arrMs: 0, lockDelayMs: 250 };
    ...
    const lockDelayInputEl = document.getElementById('lockDelayInput');
    ...
    function loadStorage() {
        try {
            ...
            return { sprint: [], settings: { ...SETTINGS_DEFAULTS } };
        }
    }

    const storage = loadStorage();
    const settings = storage.settings;
    const input = { pressed: new Map(), horizontal: 0, horizontalTimer: 0, lockResetPending: false };
    const state = { board: [], active: null, hold: '', holdUsed: false, queue: [], hasSpawned: false, lines: 0, startTime: 0, completedTime: 0, countdownUntil: 0, lastGravity: 0, sprintComplete: false, gameOver: false, lockDeadline: 0 };

    function saveStorage() {
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ sprint: storage.sprint, settings }));
    }
    ...
    function shuffleBag(opening) {
        ...
        if (!opening) return all.sort(() => Math.random() - 0.5);
        const firstPool = ['I','J','L','T'];
        const first = firstPool[Math.floor(Math.random() * firstPool.length)];
        return [first, ...all.filter(piece => piece !== first).sort(() => Math.random() - 0.5)];
    }
    ...
    function isGrounded(piece) { return !isValid({ ...piece, y: piece.y + 1 }); }
    ...
    function move(dx, dy) {
        ...
        state.active = next;
        if (isGrounded(state.active)) state.lockDeadline = performance.now() + settings.lockDelayMs;
        else state.lockDeadline = 0;
        return true;
    }
    function rotate(step, useKicks) {
        ...
        const testOffsets = !useKicks ? [[0,0]] : (state.active.type === 'I' ? kicks.I : kicks.JLSTZ)[`${from}>${to}`] || [[0,0]];
        for (const [ox, oy] of testOffsets) {
            const kicked = { ...next, x: next.x + ox, y: next.y - oy };
            if (isValid(kicked)) {
                state.active = kicked;
                state.lockDeadline = isGrounded(state.active) ? performance.now() + settings.lockDelayMs : 0;
                return true;
            }
        }
        return false;
    ...
    function clearLines() {
        let cleared = 0;
        const nextRows = [];
        state.board.forEach(row => row.every(Boolean) ? cleared++ : nextRows.push([...row]));
        while (nextRows.length < state.board.length) nextRows.unshift(Array(COLS).fill(''));
    ...
    function lockPiece() {
        ...
        getCells(state.active).forEach(cell => { if (cell.y >= 0) state.board[cell.y][cell.x] = state.active.type; });
        state.active = null;
        clearLines();
        state.lockDeadline = 0;
        if (!state.sprintComplete) spawn();
    }
    function dropOnce() {
        if (move(0, 1)) return true;
        if (!state.lockDeadline) state.lockDeadline = performance.now() + settings.lockDelayMs;
        return false;
    }
    function hardDrop() {
        while (move(0, 1)) {}
    ...
    function getGhostCells() {
        ...
        while (isValid({ ...ghost, y: ghost.y + 1 })) ghost = { ...ghost, y: ghost.y + 1 };
    ...
    function maybeStoreRecord() {
        if (!state.sprintComplete) return;
        storage.sprint.push({ nickname: '+E+RIS', timeMs: elapsed(), lines: state.lines, timestamp: new Date().toISOString() });
    ...
    function render() {
        ...
        if (state.active) {
            getGhostCells().forEach(cell => {
                if (cell.y >= HIDDEN_ROWS && !rows[cell.y - HIDDEN_ROWS][cell.x]) rows[cell.y - HIDDEN_ROWS][cell.x] = `ghost-${state.active.type}`;
            });
            getCells(state.active).forEach(cell => {
                if (cell.y >= HIDDEN_ROWS) rows[cell.y - HIDDEN_ROWS][cell.x] = state.active.type;
            });
        }
        ...
        lockDelayInputEl.value = String(settings.lockDelayMs);
        ...
        if (!state.startTime && !state.gameOver) {
            ...
            statusTextEl.textContent = 'Countdown active. +E+RIS sprint preview is now using client-side handling.';
            ...
            statusTextEl.textContent = `Client-side handling active. DAS ${settings.dasMs}ms, ARR ${settings.arrMs}ms, lock delay ${settings.lockDelayMs}ms.`;
        }
    }
    ...
    function restartHorizontalRepeat() {
        ...
        const action = input.horizontal < 0 ? () => move(-1, 0) : () => move(1, 0);
    ...
    function tick(now) {
        ...
        if (state.startTime && !state.gameOver) {
            while (now - state.lastGravity >= GRAVITY_MS) {
                dropOnce();
    ...
    settingsFormEl.addEventListener('submit', (event) => {
        event.preventDefault();
        settings.dasMs = Math.max(0, Number(dasInputEl.value || SETTINGS_DEFAULTS.dasMs));
        settings.arrMs = Math.max(0, Number(arrInputEl.value || SETTINGS_DEFAULTS.arrMs));
        settings.lockDelayMs = Math.max(0, Number(lockDelayInputEl.value || SETTINGS_DEFAULTS.lockDelayMs));
        saveStorage();
        restartHorizontalRepeat();
        render();
    });
