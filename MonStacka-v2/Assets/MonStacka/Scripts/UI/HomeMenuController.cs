    ...
    public sealed class HomeMenuController : MonoBehaviour
    {
        ...
        private static string BuildEnemyAbilityText(PieceType pieceType)
        {
            ...
            return pieceType switch
            {
                ...
                PieceType.O =>
                    $"<color=#{titleColor}><b>CALCULATED PREDICTION</b></color>\n" +
                    "<color=#FFB0B0><b>Modifier</b></color>\nCalculatedPlanning\n\n" +
                    "<color=#FFD86B><b>Trigger</b></color>\nTracks rotations across the active piece and hold swaps. Rotating more than 4 times queues a debuff for the next piece you place.\n\n" +
                    "<color=#C8FF9A><b>Effect</b></color>\nThe debuffed block permanently reduces points from any row cleared using one of its cells. Since mission damage comes from score, those rows also deal reduced mission HP damage. Higher difficulty increases the point reduction.",
                PieceType.L =>
                    $"<color=#{titleColor}><b>VISION LOSS</b></color>\n" +
                    "<color=#FFB0B0><b>Modifier</b></color>\nVision Loss\n\n" +
                    "<color=#FFD86B><b>Trigger</b></color>\nRuns on a 12s cooldown. When the cooldown fills, Vision Loss activates for 4-7s depending on story difficulty.\n\n" +
                    "<color=#C8FF9A><b>Effect</b></color>\nWhile active, every placed block on the board flickers between visible and invisible every 0.5s. It cannot be cleared early, and when it ends the board is forced visible again. The falling piece stays visible.",
                PieceType.J =>
                ...
                PieceType.S =>
                    $"<color=#{titleColor}><b>INSATIABLE HUNGER</b></color>\n" +
                    "<color=#FFB0B0><b>Modifier</b></color>\nHungerMeter\n\n" +
                    "<color=#FFD86B><b>Trigger</b></color>\nCounts the lines you clear. Early missions trigger after 3 cleared lines; harder missions reduce that requirement until every cleared line can trigger it.\n\n" +
                    "<color=#C8FF9A><b>Effect</b></color>\nAfter the trigger line clear resolves, it eats one whole block from the highest occupied layer of the board. If that block was already broken by a previous row clear, every remaining cell from that same block is still removed.",
                PieceType.T =>
                    $"<color=#{titleColor}><b>SEDATING SPIT</b></color>\n" +
                    "<color=#FFB0B0><b>Modifier</b></color>\nSedationWindows\n\n" +
                    "<color=#FFD86B><b>Trigger</b></color>\nRuns on a 15s cooldown. When it activates, it lasts 4-8s depending on story difficulty. Clearing a row ends it early.\n\n" +
